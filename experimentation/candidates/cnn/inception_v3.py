...
def inception_v3_2d(input_tensor, output_dimension=1, recurrent_layer='rnn', recurrent_params=None):
    ...
    recurrence = recurrent_layer_function(pool5_7_7, recurrent_params)
    # =====================================
    # End rnn layer insertion
    # =====================================

    loss = fully_connected(recurrence, output_dimension, activation='softmax')
    network = regression(loss, optimizer='momentum',
                         loss='categorical_crossentropy',
                         learning_rate=0.001)
    return tflearn.DNN(network, checkpoint_path='model_googlenet',
                       max_checkpoints=1, tensorboard_verbose=2)


def inception_v3_3d(input_tensor, output_dimension=1):
    """
    Generates an inception model that conforms to specified input/output tensors.
    ...
    loss = fully_connected(pool5_7_7, output_dimension, activation='softmax')
    network = regression(loss, optimizer='momentum',
                         loss='categorical_crossentropy',
                         learning_rate=0.001)
    return tflearn.DNN(network, checkpoint_path='model_googlenet',
